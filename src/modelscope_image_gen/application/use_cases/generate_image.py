...
class GenerateImage:
    ...
    async def __call__(self, request: GenerationRequest, *, max_wait_seconds: float | None = None) -> GenerateResult:
        ...
        job = submitted.job
        budget = max_wait_seconds if max_wait_seconds is not None else self._default_max_wait
        ...
        with anyio.move_on_after(budget):
            while self._monotonic() < deadline:
                ...
                if job.status is JobStatus.FAILED:
                    return GenerateResult(
                        ok=False, job=job, images=(), completed=True, partial=False, error=job.last_error
                    )
        ...
        return GenerateResult(ok=True, job=job, images=(), completed=False, partial=False)
