...
安装 `uv >=0.11.28,<0.12`，确认 `uvx` 已加入 `PATH`，并准备 Python 3.14：
...
### 3. 让 MCP Host 直接连接 GitHub 源码

当前正式安装方式直接将本仓库作为 package source，不要求用户预先 clone，也不依赖 PyPI package：
...
保存配置并重启 Host 后，应能发现五个工具。首次运行时，uv 会获取仓库、构建 package，并创建隔离且可缓存的工具环境；后续启动通常会复用缓存。

当前必须保留 `--prerelease=allow`，因为项目使用 MCP Python SDK `2.0.0b1`。只有在项目完成稳定版 SDK 的独立验证和升级后，才能移除该参数。

> [!IMPORTANT]
> PyPI 上名为 `modelscope-image-gen-mcp` 的项目由其他项目维护，不是本仓库的发行物。请使用上面的 Git URL，不要执行裸 `uvx modelscope-image-gen-mcp`。

可以先验证远端安装和 console script，而不启动长驻 MCP 会话：
...
默认的 `@main` 会跟随最新已接受源码。普通 Host 配置不要加入 `--refresh`；只有明确需要立即刷新缓存时才手工使用。需要不可变安装时，可以将 `main` 替换为受信任的 tag 或 commit。

<details>
<summary><strong>从本地源码目录进行开发</strong></summary>
...
本地 MCP Host 配置中，将下面的路径替换为源码目录绝对路径：
...
`uv --directory` 不依赖 Host 是否支持额外的 `cwd` 字段。Windows 路径建议使用正斜杠。
...
| 现象 | 处理方式 |
|---|---|
| uvx 提示无法解析 `mcp==2.0.0b1` 或 `mcp-types==2.0.0b1` | 确认 Host args 中的 `--prerelease=allow` 位于 `--from` 之前 |
| Git 安装无法获取或构建 | 检查 GitHub 访问、仓库 owner/ref、uv `>=0.11.28,<0.12` 和 Python 3.14，并在 Host 外运行文档中的 `--version` 命令 |
| Host 仍在使用旧缓存源码 | 停止 Host，给 `--version` 命令临时加入一次 `--refresh`，完成后重启；不要把 refresh 设为永久默认值 |
