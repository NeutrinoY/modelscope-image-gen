...
MockTransport 通过不能写成真实 API 已验证；真实源码运行不能替代 wheel；Registry 配置完成不能写成已经发布。未运行项是交付事实，不是可以省略的尴尬信息。
...
- [`server.json`](../../server.json) 的顶层版本和 PyPI package version 必须同步；
...
server.json top-level/package version
CHANGELOG.md
uv.lock（项目元数据）
```
...
- PyPI 与 MCP Registry 是配置完成、测试发布还是正式发布。
...
- “server.json 完成”替代“Registry 已发布”；
- “本机源码可运行”替代“wheel 用户路径可运行”；
...
- CHANGELOG 将当前 `0.2.1` 源码变化置于 `Unreleased`，并记录了真实 ModelScope、两个真实 stdio Host、wheel 和产物一致性的既有验证；
- README 只提供当前可信的源码检出运行方式；PyPI、MCP Registry、CI 平台结果和 Git 标签必须继续作为彼此独立的发布事实核对。
- PyPI 上的 `modelscope-image-gen-mcp` distribution 当前属于另一个项目；本项目在正式决定新的发行名称或完成名称协调前，不得宣传或执行裸 `uvx modelscope-image-gen-mcp` 安装。
