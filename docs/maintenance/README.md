...
- GitHub Actions 有意使用非 Hash 引用：`actions/checkout@v7` 跟随稳定 major，`astral-sh/setup-uv@v8.3.2` 使用 v8 当前可解析的不可变 release tag；实际 uv binary 仍固定为 `0.11.28`；
- 自动测试基线为 39 个默认测试及 1 个显式 live 测试；
- CHANGELOG 已将 `0.2.1` 定稿为 2026-07-12 的源码基线，并记录了真实 ModelScope、两个真实 stdio Host、wheel、远端 Git 安装和产物一致性的既有验证；
